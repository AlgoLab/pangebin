...
def compute_gc_intervals_files(
    ...
    # }
    chr_paths = open(chr_paths_file).read().splitlines()
    seq_dict = _read_FASTA_files(chr_paths, MOL_TYPE_CHR)
    pls_paths = open(pls_paths_file).read().splitlines()
    ...
    gc_endpoints = np.linspace(
        min_gc - gc_bin_size / 2,
        max_gc + gc_bin_size / 2,
        n_gcints - 1,
    )
    try:
...
def read_gc_intervals_file(gc_intervals_file):
    ...
    with open(gc_intervals_file) as in_file:
        intervals = [float(x.rstrip()) for x in in_file.readlines()]
...
def intervals_boundaries_to_str(intervals):
    ...
    """
    return [f"{intervals[i]}-{intervals[i+1]}" for i in range(len(intervals) - 1)]
...
def compute_gc_probabilities(contigs_gc, gc_intervals):
    ...
    def _gprob2(n, g, p, m):
        ...
        beta = m * (1 - p)
        resultln = (
            _combln(n, g) + sc.betaln(g + alpha, n - g + beta) - sc.betaln(alpha, beta)
        )
    ...
    for ctg_id, ctg_data in contigs_gc.items():
        ...
        total = 0
        gcp_array = []
        for i in range(len(gc_intervals) - 1):
            gp2 = integrate.quad(
...
def read_gc_probabilities_file(gcp_in_file, num_intervals=DEFAULT_N_GCINTS):
    ...
    """
    gcp_dict = {}
    with open(gcp_in_file) as in_file:
...
def compute_gc_probabilities_file(
    gfa_file,
    gc_intervals_file,
    gcp_out_file,
    gfa_gzipped=True,
