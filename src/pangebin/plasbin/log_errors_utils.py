...
def check_lists_equality(list1, list2, msg="CHECK_LISTS_EQ"):
    try:
        if set(list1) != set(list2):
            raise CustomException("Sets equality error")
    except Exception as e:
        process_exception(f"{msg}: {e}")
...
def check_lists_inclusion(list1, list2, msg="CHECK_LISTS_INCLUSION"):
    try:
        if not set(list1).issubset(list2):
            raise CustomException("Sets inclusion error")
    except Exception as e:
        process_exception(f"{msg}: {e}")
...
def check_num_fields_eq(in_list, exact_num_fields, msg="CHECK_NUM_FIELDS"):
    try:
        ...
        if num_fields != exact_num_fields:
            raise CustomException(
                f"Eaxctly {exact_num_fields} expected fields, {num_fields} read",
            )
    except Exception as e:
        process_exception(f"{msg}: {e}")
...
def create_directory(
    in_dir_list,
):  # REFACTOR to remove with using directely Path methods
...
def run_cmd_redirect(
    cmd,
    out_file_name,
    num_attempts=DEFAULT_RUN_ATTEMPTS,
    exit_on_error=True,
):
