...
from pangebin.plasbin.data_utils import read_pls_score_file
from pangebin.plasbin.gfa_fasta_utils import read_gfa_len
...
def _write_seeds_thresholds_file(thresholds, out_file_name):
    with open(out_file_name, "w") as out_file:
        out_file.write(
            "contig_length\tplasmid_score\tseed_score\tplasmids_with_seed\tfalse_seeds\tplasmids_without_seed\n",
        )
        for [
            len_thr,
            score_thr,
...
def compute_optimal_seeds_parameters(
    ...
    combined_thresholds = [
        ...
        for len_threshold in np.arange(
            min_ctg_len,
            max_ctg_len + ctg_len_step,
            ctg_len_step,
        )
        for pls_score_threshold in np.arange(
            min_pls_score,
            max_pls_score + pls_score_step,
            pls_score_step,
        )
    ]
