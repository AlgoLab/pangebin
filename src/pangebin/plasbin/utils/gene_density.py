...
def compute_gene_density(
    ...
    ctg_gd_dict = {
        ctg_id: {GD_DENSITY_KEY: 0.0, GD_INTERVALS_KEY: []} for ctg_id in ctg_len.keys()
    }
...
def _write_intervals(intervals_list):
    return " ".join([f"{start}:{end}" for [start, end] in intervals_list])
