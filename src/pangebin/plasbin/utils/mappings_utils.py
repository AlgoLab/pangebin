...
def compute_blast_qs_intervals(mappings_df):
    ...
    q_s_ids = [(q_id, s_id) for q_id in q_ids for s_id in s_ids]
    qs_intervals = {s_id: {q_id: [] for q_id in q_ids} for s_id in s_ids}
    for q_id, s_id in q_s_ids:
        q_s_hits = mappings_df.loc[mappings_df.qseqid == q_id].loc[
            mappings_df.sseqid == s_id
        ]
