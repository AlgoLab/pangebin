...
def extract_all_genes(gb_file):
    gene_collection = []
    for gb_record in SeqIO.parse(open(gb_file), "genbank"):
        # find all genes
        genes = []
        ...
        for g in genes:
            if "locus_tag" in g.qualifiers:
                gene_collection.append(
                    (g.qualifiers["locus_tag"][0], str(g.extract(gb_record.seq)))
                )
            elif "gene" in g.qualifiers:
                gene_qualifier = g.qualifiers["gene"][0]
                gene_collection.append(
                    (f"gene{i}_{gene_qualifier}", str(g.extract(gb_record.seq)))
                )
            else:
...
def create_db(
    genes_file,
    sources,
    blacklist=DEF_BLACKLIST,
    dereplicate=DEF_DEREPLICATE,
    ...
    logging.info(
        f"{len(sources)} plasmid accessions after removing blacklisted plasmids."
    )
        ...
        for i, src in enumerate(sources, start=1):
            ...
            if plasmid_seqs is not None and len(plasmid_seqs) > 0:
                ...
                if min_length <= len(plasmid_seq) <= max_length:
                    for id, seq in genes:
                        if ((not dereplicate) or (seq not in gene_seqs)) and len(
                            seq
                        ) >= min_gene_length:
                            gene_seqs.add(seq)
                            out_genes.write(">%s\n%s\n" % (id, seq))
    ...
    if num_length_discarded > 0:
        logging.warning(
            f"{num_length_discarded} plasmids were discarded for their length."
        )
    if num_attempts_discarded > 0:
        logging.warning(
            f"{num_attempts_discarded} plasmids were discarded for reaching the download-attempt limit."
        )
...
def create(
    ...
    blacklisted = []
    if blacklist != "":
        with open(blacklist) as in_file:
            for line in in_file:
    ...
    with open(from_accession) as infile:
