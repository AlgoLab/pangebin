...
import pangebin.plasbin.utils.create_db as cd
from pangebin.plasbin.gc_content import (
    DEFAULT_N_GCINTS,
    compute_gc_intervals_files,
    compute_gc_probabilities_file,
)
from pangebin.plasbin.gfa_fasta_utils import (
    gunzip_fasta,
    gunzip_GFA,
    read_fasta_id,
    read_fasta_lengths,
...
from pangebin.plasbin.utils.gene_density import (
    compute_gene_density_file,
)
from pangebin.plasbin.utils.ground_truth import (
    compute_ground_truth_file,
)
from pangebin.plasbin.utils.mappings_utils import (
    DEFAULT_COV_THRESHOLD,
    DEFAULT_PID_THRESHOLD,
    run_blast6,
)
from pangebin.plasbin.utils.seeds import (
...
def read_samples(in_csv_file, required_columns):
    ...
    try:
        ...
        missing_columns = [
            col for col in required_columns if col not in samples_df.columns
        ]
...
def create_tmp_data_files(tmp_dir, samples_df):
    ...
    """
    logging.info("## Prepare temporary FASTA/GFA files")
...
def create_ground_truth_files(
    ...
    """
    logging.info("## Compute ground truth and create new dataset CSV file")
    for sample in samples_df.index:
        logging.info(f"ACTION\tground truth for {sample}")
        logging.info("ACTION\tMapping contigs to plasmids")
        ...
        logging.info("ACTION\tcompute ground truth file")
        ground_truth_file = _ground_truth_file(out_dir, sample)
        compute_ground_truth_file(
            sample,
            pls_mappings_file,
            read_fasta_lengths(_gfa_fasta_file(tmp_dir, sample), gzipped=False),
            read_fasta_lengths(_pls_fasta_file(tmp_dir, sample), gzipped=False),
            pid_threshold,
            cov_threshold,
            ground_truth_file,
        )
...
def create_gene_density_files(
    ...
    """
    logging.info("## Compute gene density and create new dataset CSV file")
    for sample in samples_df.index:
...
def create_pls_genes_db(out_dir, tmp_dir, samples_df):
    ...
    logging.info("## Compute plasmid genes database")

    logging.info("ACTION\tcreate plasmid GenBank accessions file")
...
def map_pls_genes_to_contigs(out_dir, tmp_dir, samples_df, db_file):
    ...
    """
    logging.info("## Mapping plasmid genes database to sample contigs")
    for sample in samples_df.index:
...
def create_GC_content_intervals_file(out_dir, tmp_dir, samples_df, n_gcints):
    ...
    logging.info("## Compute GC content intervals files")
...
def create_GC_content_probabilities_files(
    ...
    """
    logging.info("## GC content probabilities files")
    for sample in samples_df.index:
        logging.info(
            f"ACTION\tcompute GC content probabilities file for sample {sample}",
        )
        gfa_file = _get_gfa(samples_df, sample)
        check_file(gfa_file)
...
def create_seeds_parameters_file(out_dir, tmp_dir, samples_df):
    ...
    logging.info("## Compute seeds parameters file")
    logging.info("ACTION\tcreate plasmids seeds input file")
    seeds_input_file = os.path.join(tmp_dir, "seeds_input.csv")
    _create_input_file(samples_df, seeds_input_file)
    logging.info("ACTION\tcreate seeds parameters file")
...
def _clean_output_files(cmd, samples_df, out_dir, tmp_dir):
    ...
    files2clean = {
        ...
        CMD_MAP_GENES_TO_CTGS: [
            _genes_mappings_file(out_dir, sample) for sample in samples_list
        ],
        CMD_GROUND_TRUTH: [
            _ground_truth_file(out_dir, sample) for sample in samples_list
        ],
        CMD_SEEDS: [
        ...
        CMD_GENE_DENSITY: [
            _gene_density_file(out_dir, sample) for sample in samples_list
        ],
        CMD_GC_PROBABILITIES: [
            _gc_proba_file(out_dir, sample) for sample in samples_list
        ],
        CMD_BINNING_MODE: [_binning_file(out_dir, sample) for sample in samples_list],
        CMD_PREDICTION: [_prediction_file(out_dir, sample) for sample in samples_list],
    }
...
def _read_arguments():
    ...
    # Map plasmid genes to samples contigs
    g2c_parser = subparsers.add_parser(
        CMD_MAP_GENES_TO_CTGS,
        parents=[argparser],
        add_help=False,
    )
    g2c_parser.set_defaults(cmd=CMD_MAP_GENES_TO_CTGS)
    g2c_parser.add_argument(
        "--db_file",
        type=str,
        help="Plasmids genes database FASTA file",
    )
    g2c_parser.add_argument(
        "--out_file",
        type=str,
        help="Path to dataset file with added mappings files",
    )
    ...
    gt_parser.add_argument(
        "--out_file",
        type=str,
        help="Path to dataset file with added ground truth files",
    )
    gt_parser.add_argument(
        "--pid_threshold",
        type=float,
        default=DEFAULT_PID_THRESHOLD,
        help="Percent identity threshold in [0,1]",
    )
    gt_parser.add_argument(
        "--cov_threshold",
        type=float,
        default=DEFAULT_COV_THRESHOLD,
        help="Percent contig coverage threshold in [0,1]",
    )
    ...
    gd_parser.add_argument(
        "--out_file",
        type=str,
        help="Path to dataset file with added gene density files",
    )
    gd_parser.add_argument(
        "--pid_threshold",
        type=float,
    ...
    gd_parser.add_argument(
        ...
        type=float,
        default=DEFAULT_COV_THRESHOLD,
        help="Percent gene coverage threshold in [0,1]",
    )
    gd_parser.add_argument(
        "--db_file",
        type=str,
    ...
    gci_parser = subparsers.add_parser(
        CMD_GC_INTERVALS,
        parents=[argparser],
        add_help=False,
    )
    gci_parser.set_defaults(cmd=CMD_GC_INTERVALS)
    gci_parser.add_argument(
        "--n_gcints",
        type=int,
        default=DEFAULT_N_GCINTS,
        help="Number of GC content intervals between 0 and 1",
    )
    ...
    gcp_parser.add_argument(
        "--out_file",
        type=str,
        help="Path to dataset file with added GC probabilities files",
    )
    gcp_parser.add_argument(
        "--gc_intervals",
        type=str,
    ...
    tuning_parser.add_argument(
        "--out_file",
        type=str,
        help="Path to dataset file with added mappings and ground truth files",
    )
    tuning_parser.add_argument(
        "--pid_threshold",
        type=float,
    ...
    preprocessing_parser.add_argument(
        "--out_file",
        type=str,
        help="Path to augmented dataset file",
    )
    preprocessing_parser.add_argument(
        "--gc_intervals",
        type=str,
    ...
    binning_parser = subparsers.add_parser(
        CMD_BINNING_MODE,
        parents=[argparser],
        add_help=False,
    )
    binning_parser.set_defaults(cmd=CMD_BINNING_MODE)
    binning_parser.add_argument(
        "--out_file",
        type=str,
        help="Path to dataset file with added binning mode files",
    )
    binning_parser.add_argument(
        "--mode",
        type=str,
        help="Binning mode to extend bins. Options: [naive, overlap]",
    )
    ...
    prediction_parser.add_argument(
        "--out_file",
        type=str,
        help="Path to dataset file with added prediction files",
    )
