...
# TODO specialize milp stats according to circular and partially circular?
class BinClassStats(YAMLInterface):
    """Bin class MILP stats."""

    KEY_OBJECTIVE_VALUE = "objective_value"


@final
class BinStats(YAMLInterface):
    """Bin milp stats."""
    ...
    @classmethod
    def from_dict(cls, obj_dict: dict) -> BinStats:
    ...
    def __init__(
        self,
        total_flow: float,
        min_non_zero_arc_flow: float | None,
    ...
    def min_non_zero_arc_flow(self) -> float | None:
...
def stats_from_opt_bin_vars(
    network: net.Network,
    bin_vars: lp_vars.BinVariables,
    obj_fun_domain: cmn_lp_objs.ObjectiveFunctionDomain,
) -> BinStats:
    """Create MILP stats from optimal variables."""
    min_non_zero_arc_flow = min(
        (
    ...
    min_non_zero_inflow = min(
        bin_vars.flows().incoming_forward_reverse(network, frag_id).getValue()
        for frag_id in network.fragment_ids()
        if bin_vars.flows().incoming_forward_reverse(network, frag_id).getValue() > 0
    )

    return BinStats(
        bin_vars.flows().total().X,
        min_non_zero_arc_flow,
        min_non_zero_inflow,
        lp_obj.plasmidness_score(network, bin_vars.flows(), obj_fun_domain).getValue(),
    )
