...
ASS_PENALTY_KEY = "assembly_penalty"
SEED_KEY = "seed"
...
def read_ctgs_data(
    ...
    """Read all data related to contigs.

    Args:
        - in_gfa_file (str): path to a gzipped GFA file
        - in_pls_score_file (str): path to a plasmid score file
        - assembler (Assembler): tag of the used assembler
    ...
    ctgs_len_dict = read_gfa_len(gfa_filepath)
    ctgs_cov_dict = read_gfa_normalized_coverage(
        gfa_filepath,
        assembler,  # REFACTOR should have a default assembler value?
    )
    ctgs_asspen_dict = read_gfa_ass_penalty(
        gfa_filepath,
    )  ## TODO Check this
    _update_ctgs_dictionary(ctgs_data_dict, ctgs_len_dict, LEN_KEY)
    _update_ctgs_dictionary(ctgs_data_dict, ctgs_cov_dict, COV_KEY)

    ## ADDING PENALTY TERM FOR PANGENOME EDGEs AND FRAGMENTS
    _update_ctgs_dictionary(ctgs_data_dict, ctgs_asspen_dict, ASS_PENALTY_KEY)

    ctgs_score_dict = read_pls_score_file(plasmid_score_filepath)
    check_lists_inclusion(
        ctgs_score_dict.keys(),
        ctgs_len_dict.keys(),
        msg=f"{plasmid_score_filepath} has contigs not present in {gfa_filepath}",
    )
    ...
    for ctg_data in ctgs_data_dict.values():
        if SCORE_KEY not in ctg_data or ctg_data[LEN_KEY] < min_ctg_len:
...
def get_seeds(
    ...
    """Compute the set of seed contigs and updates the contigs data.
    ...
    for ctg_id, ctg_data in ctgs_data_dict.items():
        ...
        logging.info(f"contig_len: {ctg_data[LEN_KEY]}")
        logging.info(f"contig_score: {ctg_data[SCORE_KEY]}")
...
def read_gc_data(gc_probabilities_file, gc_intervals_file, gfa_ctgs_list):
    """Read the GC probabilities and intervals.
    ...
    __gc_probs_dict = read_gc_probabilities_file(gc_probabilities_file, num_intervals)
...
def read_links_data(in_gfa_file: Path):
    ...
    links_list = []
    __links_dict = read_GFA_links(in_gfa_file)
    for ctg_id, ctg_links_from_list in __links_dict.items():
        for link in ctg_links_from_list:
            ext_from = (ctg_id, ctg_from_ext[link[GFA_FROM_ORIENT_KEY]])
            ext_to = (link[GFA_TO_KEY], ctg_to_ext[link[GFA_TO_ORIENT_KEY]])
...
def log_data(ctgs_data_dict, links_list, in_gfa_file, in_pls_score_file):
    ...
    logging.info(
        f"DATA\tFile {in_pls_score_file} maximum plasmid score {max(score_list)}",
    )
    num_seeds = len(
